...
void LIS3DSH_freefall_init(void)
{
	...
	//LIS3DSH_write(TIM1_1L,0x07); //Free-fall duration (= 25 ms)
	//LIS3DSH_write(TIM1_1L,0x07); //Free-fall duration (= 50 ms)
	LIS3DSH_write(TIM1_1L,0x28); //Free-fall duration (= 100 ms)

	LIS3DSH_write(THRS2_1,0x18); //Free-fall threshold (= 375 mg)
	//LIS3DSH_write(THRS2_1,0x32);   //Free-fall threshold (= 500 mg)
	//LIS3DSH_write(THRS2_1,0x64);   //Free-fall threshold (= 1 g)
...
{
	int ax, ay, az;
	int8_t RxData[6] = {0}; //changed to int since sensor output data is signed
	...
	LIS3DSH_init();
	//LIS3DSH_freefall_init();  //didn't worked with interrupt in my case
	...
	while(1)
	{
		//using sing IO with SPI since got error with multibytes
		...
		/* fall detection with polling, it detect fall but could not distinguish between forced-fall and free-fall
